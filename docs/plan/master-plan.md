...
4. **已拍板（2026-07-07，fresh-context 對抗審查後修正）**：(a) 內容類型判別**擴充既有 category
   slug 模式、不加 `items.type` 欄位**（見下方「共通設計決策」）；(b) DealInfo 審核狀態名用
   `pending_review`（對齊 `ItemStatus` 既有語意，不用 `draft`），審核佇列納入交付內容 2；
   (c) `keyword_blocklist` 詞庫寫入**seed upsert＋後台 CRUD 頁兩者都做**（見交付內容 3）；
   (d) 票券使用期限**沿用 `items.expires_at`**，`ticket_details` 不設自己的效期欄位。

### 交付內容

**共通設計決策（已拍板）：內容類型判別機制——擴充既有 category slug 模式，不加 `items.type`
欄位。** 理由：M3 優惠券已用「category slug（`coupons`）＋ detail 表（`coupon_details`）＋
表單依 slug 切換子表單＋API 依 slug 驗證」跑通整條線；擴充這個模式零 `items` schema 變更、
無雙機制並存的混淆。票券/點數完全比照：
- (a) `src/lib/categories.ts` 新增 `TICKET_CATEGORY_SLUG`／`POINT_CATEGORY_SLUG` 常數（比照
  既有 `COUPON_CATEGORY_SLUG`）。
- (b) `prisma/seed.ts` 新增 `tickets`／`points` 兩個分類的 idempotent seed（upsert，重複執行
  不重複建立）。
- (c) `item-form.tsx` 依 slug 顯示票券/點數子表單（比照優惠券子表單既有慣例）。
- (d) `POST /api/items` 依 slug 驗證對應必填欄位、**拒絕任何金額欄位**。
- (e) **已知取捨**：「分類即類型」的語意侷限（例如票券物品不能同時屬於娛樂等其他分類）沿襲
  M3 優惠券既有取捨；未來若要真正的 `items.type` 欄位屬獨立重構，**不在 M9 範圍**。
...
審核狀態名用 `pending_review` **對齊 `ItemStatus` 既有語意**，不另創 `draft`）：
```
pending_review -- REQUIRE_REVIEW flag 開啟時，使用者投稿先進此態，進交付內容 2 的審核佇列；
                  flag 關閉時投稿直接 published；編輯（moderator/admin）自建一律直接 published
published      -- 上架中，公開可見、產生 SEO metadata
stale          -- 失效回報達門檻，自動標記（可 reactivate 回 published，見下方失效回報）
expired        -- 到期日已過（硬性 TTL），終態
rejected       -- 審核駁回，終態（僅能由 pending_review 轉入）
```
轉換：`pending_review→published`（審核核准）、`pending_review→rejected`（審核駁回，終態）、
`published→stale`（失效回報達門檻）、`stale→published`（reactivate）、`published→expired`／
`stale→expired`（TTL 到期）。expired／rejected 為終態（不可 reactivate；已過期的好康沒有救回
意義）。跳態或逆向轉換一律 409（比照 M2 檢舉狀態機慣例）。`pending_review` 的 DealInfo 對非
投稿者與非 moderator/admin 一律 404、不產生 SEO metadata（比照 M2 `REQUIRE_REVIEW` 對物品
詳情頁的既有處理）。
...
- `deal_info_reports`（失效回報，一人一物一次；防單人惡意殺文靠 unique＋門檻 ≥2）：
  ...
  round         int @default(1)  -- 第幾輪回報（reactivate 後 +1，見下方），配合 unique 允許同人跨輪再報
  created_at
...
#### 2. 編輯人工收錄（方案 B 的 S1 來源）——後台「來源管理」＋DealInfo 審核佇列
...
- DealInfo（`source_type=editorial`）以 `deal_source_id` 關聯 `deal_sources`，詳情頁顯示來源名稱＋
  該來源 `last_checked_at`。
- **DealInfo 審核佇列（最小集）** `/admin/deal-reviews`（moderator/admin，比照
  `/admin/support-tickets` 後台既有慣例，直接查 db）：
  - `REQUIRE_REVIEW` flag **開啟**時，使用者投稿的 DealInfo 進 `pending_review` 並出現在此
    佇列；佇列提供列表＋**核准**（轉 `published`、寫 `published_at`）＋**駁回**（轉 `rejected`，
    終態），狀態轉換皆寫 `audit_logs`。
  - flag **關閉**時投稿直接 `published`——既有防線兜底：rate limit＋關鍵字黑名單＋檢舉＋
    強制下架。
  - 編輯（moderator/admin）自建的 DealInfo **一律直接 `published`、不進佇列**。
  - `admin-nav.tsx` 補入口（與 `/admin/deal-sources` 並列）。

#### 3. 券類強化（give-to-get 配額＋使用結果回報＋不可上架清單）

延續 M3 既有優惠券（AES-256-GCM 券碼加密＋領取後揭露），本交付補三項：

- **give-to-get 領取配額掛既有貢獻值**（研究 02 CouponShare 借鏡 #3）：券票點三類物品的每日
  「留言/認領」額度依使用者累計貢獻值分級（例：新手每日 N 次；分享過券後提高），超額回 429
  （沿用 M2 rate limit 錯誤格式）。**實作註記（審查修正）**：
  - 既有 `src/app/api/items/[id]/claims/route.ts` 目前**沒有任何 category/coupon 判斷邏輯**
    （審查者已逐行確認），本交付需**新增**判斷邏輯：claims route 查詢 item 時一併帶出 category
    slug（或 `couponDetail`／`ticketDetail`／`pointDetail` 存在性），僅對券票點三類套用配額
    檢查，**實體物品不受影響**。
  - `src/lib/contribution.ts` 目前**沒有**累計分數查詢 helper，需**新增**對
    `contribution_events` 的 SUM 聚合（快取或直接聚合由實作依 §11.4 快取慣例決定，
    **`User` 表不加欄位**）。
...
- **不可上架清單（雙重攔截，分工明確）**（研究 02 台灣在地化 #2、票券結論；研究 01 §4）：
  - **硬擋範圍（使用者 2026-07-07 拍板，兩階段收斂後定案）：全面清空、一項都不硬擋**。
    LINE 即享券、LINE 禮物、7-ELEVEN 行動隨時取、全家隨買跨店取等官方閉環/禁轉贈類
    一律**可上架**，改以詳情頁文案（轉贈風險提示＋「能否轉讓依發行人條款」）承擔風險溝通、
    引導走官方轉贈功能。歷史脈絡：原稿曾誤列裸詞「即享券」（Edenred 通用票券品牌，多數可
    自由轉贈、正是平台利基，2026-07-07 依 code review 查證先收斂為僅 LINE 即享券），同日
    使用者再拍板全面清空。
  - **攔截層一（表單/API 常數清單）——擋「類型選擇＋標題」的機制保留、清單為空**：
    正規化（去空白、全形轉半形、英文小寫化）＋子字串比對的程式邏輯不拆除，日後若要恢復
    硬擋任何券種，把詞條加回常數清單即可（`src/lib/non-transferable-coupon-types.ts`、
    `src/lib/ticket-guard.ts`）。
  - **攔截層二（`keyword_blocklist`）——負責擋「自由文字」（描述、留言）**：擴充 M2
    `keyword_blocklist` 詞庫，命中即 422（沿用既有黑名單機制，不新增攔截程式）。
- **M9 `keyword_blocklist` 詞庫與管理（已拍板：兩者都做，皆納入本交付）**：
  - (a) **seed**：`prisma/seed.ts` 加 M9 詞庫的 **idempotent upsert**（重複執行不重複建立），
    詞條四組：**加價詞**（「+300」「小補」「私訊出價」「補差價」「加價」）、**折現詞**
    （「折現」「換現金」「交換」「面交補」）、**不可上架券名**（依使用者 2026-07-07 決策
    **全面清空、一條都不 seed**；seed 內含一次性清理，會刪除先前種下的「即享券」與 LINE/
    閉環類詞條，理由見上方攔截層說明）、**個資徵求詞**（「驗證碼」「會員帳號」「OTP」）。
  - (b) **後台 KeywordBlocklist CRUD 頁** `/admin/keyword-blocklist`（moderator/admin）：
    `keyword_blocklist` 表已存在（`prisma/schema.prisma:780-787`）但至今無管理頁——本來就在
    研究 01「可立即修正」清單 #6；比照既有 `/admin/*` 頁慣例直接查 db，異動寫 `audit_logs`；
    `admin-nav.tsx` 補入口。
...
handover 交付、不做獨立五類型狀態機重構——呼應研究 01 的漸進建議；類型判別依上方「共通設計
決策」採 category slug（`tickets`）＋detail 表模式）。
...
- 新增 `ticket_details`（掛在對應 `items` 列上，比照 `coupon_details` 慣例）：
  ...
  created_at / updated_at
  ```
  **明確無「金額」欄位**（研究 04：任何金額/補貼名義收費即違法）。**使用期限已拍板沿用
  `items.expires_at`**（單一事實來源，M3 既有 `item-expiration` 到期 job 直接生效），
  `ticket_details` **不設**自己的效期欄位。
...
claims/handover 交付；類型判別依上方「共通設計決策」採 category slug（`points`）＋detail 表
模式）。
...
- **個資最小化（硬規則）**：平台**不記錄任何會員帳號／手機號／驗證碼**——這些欄位**明文禁止**
  出現在表單與留言，攔截分兩層（**審查修正：不可假設既有機制能攔格式**）：
  - **固定詞**（「驗證碼」「會員帳號」「OTP」等）：擴充 M2 `keyword_blocklist`（詞條見交付
    內容 3 的 seed 清單）。既有 `checkKeywordBlocklist` **只做子字串比對**（程式碼註解明載），
    攔固定詞可行、攔格式不可行。
  - **手機號格式**：需**新增獨立的正則檢查 helper**（例如台灣手機 `09\d{8}` 及其全形數字、
    帶分隔符「0912-345-678」等變體），套用在點數類的表單欄位與留言內容，命中即 422。
  轉移的帳號級操作只發生在雙方各自的官方 App 內，平台不觸及。
...
- **正式站 `REQUIRE_REVIEW` 是否開啟**（研究 01「不確定，需查證」）——影響 DealInfo 投稿是否
  先進 `pending_review`（物品側則沿用 M2 既有行為）。
...
- [ ] `prisma/seed.ts` 重複執行兩次皆成功且 idempotent：`tickets`／`points` 兩個分類、10 個 S1
      來源、M9 `keyword_blocklist` 四組詞條皆恰好一份、不重複建立。
- [ ] `deal_infos` 列表查詢（狀態＋縣市/全台篩選＋cursor 分頁）以 500+ 筆假資料跑
      `EXPLAIN ANALYZE`，走 §11.2 新建索引（Index Scan，非 Seq Scan）——比照 M1 驗收慣例。
...
- [ ] DealInfo 審核佇列：`REQUIRE_REVIEW` 開啟時使用者投稿進 `pending_review`，對非投稿者/非
      moderator/admin 一律 404、不產生 SEO metadata；`/admin/deal-reviews` 核准後轉 `published`
      （`published_at` 有值）、駁回後轉 `rejected`（終態，不可再轉出）；flag 關閉時投稿直接
      `published`；moderator/admin 自建的 DealInfo 一律直接 `published` 不進佇列；核准/駁回皆
      寫 `audit_logs`。
- [ ] `/admin/keyword-blocklist`：moderator/admin 可 CRUD 詞條、異動寫 `audit_logs`；新增詞條後
      立即在上架/留言攔截生效；非 moderator/admin 存取 → 404/403。
- [ ] 券類 give-to-get 配額：低貢獻值帳號在當日額度用盡後，第 N+1 次券類認領回 429；分享過券
      提高額度後可繼續；**實體物品的認領完全不受配額影響**（claims route 新增的類型判斷只套用
      券票點三類）。
- [ ] 券使用結果回報：同一使用者對同一券回報兩次 → 第二次被 unique 擋下；詳情頁聚合統計
      （可用/已失效人數）正確；**文案無「保證有效/保證可兌換」字樣**。
- [ ] 不可上架清單（使用者 2026-07-07 拍板全面清空）：「LINE 即享券」「隨買跨店取」
      「行動隨時取」「LINE 禮物」「麥當勞即享券」等皆可正常上架、不硬擋；攔截機制程式
      邏輯保留（清單為空），加價詞/折現詞/個資徵求詞的 keyword_blocklist 攔截不受影響。
- [ ] 票券類型：`ticket_details` 無金額欄位、無自己的效期欄位（效期沿用 `items.expires_at`，
      到期由 M3 既有 `item-expiration` job 直接生效）；上架帶「+300」「小補」「私訊出價」→ 422
      （關鍵字攔截）；票券詳情頁標配文創法/運動條例警示與轉贈風險提示；`origin_platform=KKTIX`
      顯示「走官方轉讓」說明；確認後能無痛接續既有 `handover/ensure` 交接。
- [ ] 點數類型：`point_details` 無金額欄位、僅 `point_platform`＋`point_amount`；表單/留言含
      「驗證碼」「會員帳號」→ 422（`keyword_blocklist` 固定詞攔截）；含台灣手機號（`09xxxxxxxx`
      及全形/帶分隔符變體）→ 422（**新增的正則檢查 helper** 攔截，非 blocklist）；含「折現」
      「換現金」「交換」→ 422；詳情頁標配「以官方 App 為準、本平台不經手點數」警示；**全站
      grep 確認無任何「平台代轉/代管點數」邏輯或欄位**；交付沿用既有 claims/handover。
...
- [ ] read-back 全章：無任何自動爬取規劃、無「麥當勞報報」舊名、家樂福一律用新名「萬家福」、
      點數段落無任何「平台代轉點數」設計、無「既有券類分支」等暗示既有 claims route 已有類型
      判斷的描述、DealInfo 狀態機無 `draft` 字樣。
- [ ] `docs/governance/judgment-rubrics.md` §5 三組底線逐條過（比照 M1 驗收慣例）。
...
- M9（草案，待使用者確認，見 §9a）：deal_infos, deal_info_cities, deal_info_reports, deal_sources,
  ...
  claims/handover；類型判別採 category slug 模式——`tickets`／`points`，不加 `items.type`
  欄位，見 §9a 共通設計決策；DealInfo 獨立成表因無交付）
- 後移未排：badges/user_badges/leaderboard_*（徽章排行榜——等有活躍使用者再說）、
  sensitive_access_logs（M2 併入 audit_logs 加 `sensitive` 欄位起步，量大再拆表）

### 11.8 未來功能參考（競品觀察，2026-07-06；原誤編 11.5 與「資料保留」重號，2026-07-07 更正，全 repo 無其他引用處）
...
（M9 草案）點數/會員資料最小化→表單與留言禁存會員帳號/手機號/驗證碼（固定詞走 keyword_blocklist、
手機號格式走新增正則 helper）、平台不經手點數；票券/點數禁對價→無金額欄位＋加價/折現關鍵字攔截；
外部來源→僅人工收錄自寫摘要＋導流，禁自動爬取（Lawsnote 風險）。
