...
- **不可上架清單（雙重攔截，分工明確）**（研究 02 台灣在地化 #2、票券結論；研究 01 §4）：
  - **官方禁轉贈/官方閉環類型**：LINE 禮物／LINE 即享券（官方明文禁轉贈）、7-ELEVEN 行動
    隨時取、全家隨買跨店取（官方閉環，正確做法是引導走官方轉贈功能，不在本平台上架）。
  ...
  - **攔截層一（表單/API 常數清單）——負責擋「類型選擇＋標題」**：程式內定一份「不可上架
    類型」常數清單，券票類上架時把輸入**正規化（去空白、全形轉半形、英文小寫化）後做子字串
    比對**，已知變體一併列入清單（「LINE即享券」「LINE 即享券」「line即享券」「LINE 禮物」
    「line gift」「隨買跨店取」「行動隨時取」等），命中即 422 拒絕（附說明「此類型為官方
    閉環/禁轉贈，請走官方 App 轉贈功能」）。
...
- **M9 `keyword_blocklist` 詞庫與管理（已拍板：兩者都做，皆納入本交付）**：
  - (a) **seed**：`prisma/seed.ts` 加 M9 詞庫的 **idempotent upsert**（重複執行不重複建立），
    ...
    （「折現」「換現金」「交換」「面交補」）、**不可上架券名**（「LINE即享券」「LINE 即享券」
    「LINE 禮物」「隨買跨店取」「行動隨時取」；此表比對無空白正規化，空白變體各列一條；
    **不得收裸詞「即享券」**，理由見上方攔截層說明）、**個資徵求詞**（「驗證碼」「會員帳號」
    「OTP」）。
