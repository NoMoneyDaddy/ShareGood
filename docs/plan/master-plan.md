...
data_export_id     FK -> data_exports.id, nullable  -- type=data_export 時關聯
...
action          RetentionAction  -- purge | anonymize | downgrade | archive
...
`HandoverRecord.completedAt <= now() - retention_days` 的物品底下的 `Message`），對每筆命中
資料先 `isUnderLegalHold` 檢查，未命中才執行 `action`，每筆動作寫 `data_purge_logs`。
