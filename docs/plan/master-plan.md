...
- **不可上架清單（雙重攔截，分工明確）**（研究 02 台灣在地化 #2、票券結論；研究 01 §4）：
  - **硬擋範圍（使用者 2026-07-07 拍板）：僅「LINE 即享券」一項**。
    「LINE 禮物」「7-ELEVEN 行動隨時取」「全家隨買跨店取」等官方閉環類**不攔截**，改以
    詳情頁文案引導走官方轉贈功能。
    **注意：只擋「LINE 即享券」，不得擋裸詞「即享券」**——「即享券」是 Edenred 的通用電子
    票券品牌（麥當勞/SOGO/家樂福即享券等），多數為可自由轉贈的序號券、正是本平台利基；
    研究 04 查證到的禁轉贈依據僅限 LINE（2026-07-07 依 code review 查證修正，原稿曾誤列
    裸詞）。
  - **攔截層一（表單/API 常數清單）——負責擋「類型選擇＋標題」**：程式內定一份「不可上架
    類型」常數清單（現行內容即「LINE即享券」一條，正規化涵蓋空白/大小寫變體），券票類上架
    時把輸入**正規化（去空白、全形轉半形、英文小寫化）後做子字串比對**，命中即 422 拒絕
    （附說明「此類型為官方閉環/禁轉贈，請走官方 App 轉贈功能」）。
...
- **M9 `keyword_blocklist` 詞庫與管理（已拍板：兩者都做，皆納入本交付）**：
  - (a) **seed**：`prisma/seed.ts` 加 M9 詞庫的 **idempotent upsert**（重複執行不重複建立），
    ...
    （「折現」「換現金」「交換」「面交補」）、**不可上架券名**（僅「LINE即享券」「LINE 即享券」
    兩條；此表比對無空白正規化，空白變體各列一條；**不得收裸詞「即享券」**，「LINE 禮物」
    「隨買跨店取」「行動隨時取」依使用者 2026-07-07 決策不收，理由見上方攔截層說明）、
    **個資徵求詞**（「驗證碼」「會員帳號」「OTP」）。
...
- [ ] 不可上架清單：以「LINE 即享券」（含空白/半形變體）為標題/券種上架 → 422（表單常數
      清單＋`keyword_blocklist` 任一命中即擋）；「隨買跨店取」「行動隨時取」「LINE 禮物」
      可正常上架（使用者 2026-07-07 拍板不攔截）；「麥當勞即享券」等通用即享券可正常上架。
