...
> 讀法：開工前讀 §1–§3（定位與通用慣例），然後**只讀你目前 milestone 的那一節**。不要試圖一次讀完全文。
...
COUPON_SECRET_KEY       # M4 起：券碼加密（AES-256-GCM）
TELEGRAM_BOT_TOKEN / TELEGRAM_WEBHOOK_SECRET              # M5 起
...
1. 資料表：`reports`、`report_evidence`、`user_restrictions`、`item_removals`、
   `support_tickets`（含附件表）、`keyword_blocklist`。
...
其餘隨對應 milestone。`/rules`、`/terms`、`/privacy` 三個靜態頁在 §12 上線前補。
